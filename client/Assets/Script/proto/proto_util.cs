...
public class proto_util
{
    ...
    public static Int64 readLong(MemoryStream data)
    {
        Int64 tmpvalue = 0;

        byte[] nowData = data.GetBuffer();
        tmpvalue = BitConverter.ToInt64(nowData, (int)data.Position);
		tmpvalue = IPAddress.NetworkToHostOrder(tmpvalue);
        readStream(data, sizeof(long));
    ...
    public static UInt64 readULong(MemoryStream data)
    {
        ...
        int _end = (int)data.Position + sizeof(Int64);
    ...
    {
        int Len = readShort(data);
        for (int i = 0; i < Len; i++) _list.Add(readULong(data));
    }

    public static void readLoopLong(MemoryStream data, List<long> _list)
    {
        int Len = readShort(data);
        for (int i = 0; i < Len; i++) _list.Add(readLong(data));
    }
    ...
    public static void writeLong(MemoryStream data, Int64 tmpvalue)
    {
        tmpvalue = IPAddress.HostToNetworkOrder(tmpvalue);

        data.Write(BitConverter.GetBytes(tmpvalue), 0, sizeof(Int64));
    }
...
//            data.Write(nowData, 0, sizeof(UInt64));
		UInt64 lh = (UInt64)IPAddress.HostToNetworkOrder ((Int64)tmpvalue);
		byte[] nowData = System.BitConverter.GetBytes (lh);
		data.Write(nowData, 0, sizeof(UInt64));
    }
    ...
    public static void writeLoopULong(MemoryStream data, List<ulong> list)
    {
        ...
        proto_util.writeShort(data, (short)list.Count);
        foreach (ulong ele in list) writeULong(data, ele);
    }

    public static void writeLoopLong(MemoryStream data, List<long> list)
    {
        proto_util.writeShort(data, (short)list.Count);
        foreach (long ele in list) writeLong(data, ele);
    }
